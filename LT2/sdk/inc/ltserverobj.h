...
#define __LTSERVEROBJ_H__
	...
	#define MID_ACTIVATING		12
	
	// Sent when an object is becoming inactive
	#define MID_DEACTIVATING	13
	...
	#define END_CLASSDEFS2() \
		...
		BEGIN_EXTERNC() \
			__declspec(dllexport) ClassDef** ObjectDLLSetup(int *nDefs, ILTServer *pServer, int *version); \
		END_EXTERNC() \
	...
	// so you don't have to add each object to the list.
		...
		#define DEFINE_CLASSES() \
			...
			ILTServer *g_pLTServer=(ILTServer*)0;\
			BEGIN_EXTERNC() \
				__declspec(dllexport) ClassDef** ObjectDLLSetup(int *nDefs, ILTServer *pServer, int *version); \
			END_EXTERNC() \
